...
public class ToitOverrideMethodsHandler implements LanguageCodeInsightActionHandler {
    @Override
    ...
    public void invoke(@NotNull Project project, @NotNull Editor editor, @NotNull PsiFile file) {
        ...
        if (structure!= null && (structure.isClass() || structure.isMonitor())) {
            List<ToitFunction> override = OverrideImplementUtil.getFunctionsToOverride(structure);
            if (override.isEmpty()) {
                HintManager.getInstance().showErrorHint(editor, "No methods to override");
                return;
            }

            OverrideImplementUtil.chooseAndOverrideImplementMethods(project, editor, structure,override,false);
        }
    }
}
